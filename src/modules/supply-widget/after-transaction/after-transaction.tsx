...
import { BulkerUSDCABI } from '@/shared/web3/abi/BulkerUSDCABI';
...
const AfterTransaction = ({
  ...
  const onClickTransaction = async () => {
    ...
    if (currentTokenData.isNative && isChecked) {
      ...
      buyTokensWithETH(
        {
          ...
          ...(response?.estimateGas && feeData?.maxFeePerGas && { gasPrice: feeData.maxFeePerGas }),
      ...
      buyTokensWithETH(
        {
          abi: currentTokenData.unique ? BulkerUSDCABI : BulkerABI,
          address: currentTokenData.bulker,
          functionName: 'invoke',
          args: [
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-expect-error
            [currentTokenData.unique ? ACTION_SUPPLY_TOKEN_USDC : ACTION_SUPPLY_TOKEN],
            [supplyToken],
          ],
          ...(response?.estimateGas && feeData?.maxFeePerGas && { gasPrice: feeData.maxFeePerGas }),
          ...(response?.estimateGas && { gas: response.estimateGas }),
