...
const EditCollateral: FC<EditCollateralProps> = ({
  ...
  const minLiqRisk = useMemo(() => {
    ...
    return Number(response.toFixed(1));
  }, [currentCollateralData]);

  const minAvailableHF = useMemo(() => {
    return minLiqRisk + 0.1;
  }, [minLiqRisk]);
  ...
  const isDisabled = useMemo(() => {
    ...
    if (collateralAction === 'minus') {
      return !(
        Number(value) > 0 &&
        Number(value) <= Number(maxBalance) &&
        Number(liqRisk) >= Number(minAvailableHF.toFixed(1))
      );
    }
  ...
  const onMax = useCallback(() => {
    if (collateralAction === 'minus' && borrowAmount > 0) {
      setLiqRisk(minAvailableHF.toFixed(1));

      calculateCollateralsOnChangeHF(minAvailableHF);
      return;
    }
  ...
  }, [maxBalance, currentCollateralAmount, minAvailableHF]);
  ...
  const onLiqRiskChange = useCallback(
    (target: string) => {
      ...
      if (
        ...
        const adjustedHealthFactor = currentHealthFactor.toFixed(1);
              ...
              >
                ...
                >
                  <Text size='medium18500120'>
                    Current Health Factor {currentHealthFactor.toFixed(1)}
                  </Text>
                  <Text
                    size='small14500120'
                    color='brand.550'
                  >
                    {collateralAction === 'plus'
                      ? `Max Health Factor ${maxLiqRisk.toFixed(1)}`
                      : `Min Health Factor ${minLiqRisk.toFixed(1)}`}
                  </Text>
                </Flex>
